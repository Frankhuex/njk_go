...
func patternSources(botUserID string) []string {
	return []string{
		`^ *\.概括 *(\d+) *$`,
		`^ *\.总结 *(\d+) *$`,
		`^ *\.俳句 *(\d+) *$`,
		...
		`^ *\.xmas *(\d+) *$`,
		`^ *\.ai *(\d+) *$`,
		fmt.Sprintf(`你居垦|\[CQ:at,qq=%s\]`, botUserID),
		`^ *(?:\.aic|\.ai *c) *$`,
		`^ *\.报告 *(\d+) *$`,
		`^ *\.help *$`,
		`^ *\.help *bbh *$`,
...
不要用markdown，请输出纯文本`,
	...
	`分析以下内容，找出其中两个主要人物，改写成这两个人之间的对决较量，以小说的文笔描写，情节内容要影射出这些聊天记录的内容，风趣幽默，让人忍俊不禁。
